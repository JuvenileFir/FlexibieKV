...
static void mehcached_zipf_init(struct zipf_gen_state *state, uint64_t n, double theta, uint64_t rand_seed) {
  assert(n > 0);
  if (theta > 0.992 && theta < 1) fprintf(stderr, "theta > 0.992 will be inaccurate due to approximation\n");
  if (theta >= 1. && theta < 40.) {
...
static void mehcached_zipf_init_copy(struct zipf_gen_state *state, const struct zipf_gen_state *src_state,
                                     uint64_t rand_seed) {
  assert(rand_seed < (1UL << 48));
...
static void mehcached_zipf_change_n(struct zipf_gen_state *state, uint64_t n) { state->n = n; }

static double mehcached_zeta(uint64_t last_n, double last_sum, uint64_t n, double theta) {
  if (last_n > n) {
    last_n = 0;
    last_sum = 0.;
  }
...
static void mehcached_test_zipf(double theta) {
  ...
  if (theta < 1. || theta >= 40.) printf(", using approx-pow(): %.10lf", (double)num_key0 / (double)num_samples);
  printf("\n");
}
