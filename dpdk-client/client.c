...
*/
    ...
    uint32_t queue_id = context->queue_id;
    uint16_t i, port = 0;
    struct rte_mbuf *tx_bufs_pt[PKG_GEN_COUNT * SEND_ROUND];//用来存储packet们
...
#else
    uint16_t pktlen = length_packet[WORKLOAD_ID];
#endif
   ...
   */
    ...
    for (int i = 0; i < PKG_GEN_COUNT * SEND_ROUND; i++) {//总共申请多少个packet的内存空间(bwb:目前20个)
        ...
        ip_hdr->packet_id = 0;
        ip_hdr->total_length = rte_cpu_to_be_16(pktlen);//将16位值从CPU顺序转换为大端
...
static void tx_pkt_load(char *ptr, struct zipf_gen_state *zipf_state,
    ...
    uint64_t k, get_key, set_key = *(uint64_t *)(start_set_key + t_id);//开始时1
    // printf("set_key begin:%ld\n",set_key);
    //在一个包内，get请求是先type，然后keylen，然后key
    //set的话，先是type，然后klen，valueLen，然后key和value
    for (k = 0; k < number_packet_set[WORKLOAD_ID]; k++) {//设定的set次数//bwb:number_packet_set[WORKLOAD_ID]=40???
        *(uint16_t *)ptr = MEGA_JOB_SET;//#define MEGA_JOB_SET 0x3
        ptr += sizeof(uint16_t);//向后移动2 byte 即操作类型2 byte
        *(uint16_t *)ptr = KEY_LEN;//2 byte
        ptr += sizeof(uint16_t);
        *(uint32_t *)ptr = VALUE_LEN;//4 byte
        ptr += sizeof(uint32_t);

        set_key++;
        
        // if (set_key >= (uint64_t)(TOTAL_CNT)) {
        /* if (set_key >= ((uint64_t)65536*(uint64_t)1024)||set_key <= 10 ){
            printf("OUT!!!!%ld\n",set_key);
            // set_key = 1;
        } */
        if (set_key >= (uint64_t)QUARTER_CNT*(t_id + 1)) set_key = 1 + t_id * (uint64_t)QUARTER_CNT;
        // if (set_key >= ((t_id+2)*(uint64_t)1000000000)) set_key = (t_id+1)*(uint64_t)1000000000;
        
        //超出64位所能表示的范围则从1重新开始
        set_key = 1;//xhj 1021
        *(uint64_t *)(ptr) = set_key;//16+16+32=64,前面正好用了64位，set的key是从2开始逐渐递增的
        ptr += KEY_LEN;//向后移动KEY_LEN,也就是bbyte
        // *(uint64_t *)(ptr) = set_key + 1;//value是key+1
        *(uint64_t *)(ptr) = rand();//value是rand()
        ...
        *(uint16_t *)ptr = KEY_LEN;
        ...
        // get_key = (uint64_t)mehcached_zipf_next(zipf_state) + 1;
        ...
        *(uint64_t *)(ptr) = get_key;
        ptr += KEY_LEN;//+8字节，即64位
    }
    *(uint64_t *)(start_set_key + t_id) = set_key;//更新start_set_key字段
    /* pkt ending flag */
    *(uint16_t *)ptr = 0xFFFF;
}
