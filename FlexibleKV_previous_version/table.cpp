...
void hash_table_init(hash_table *table, DataStore *stores) {
  table->num_partitions = shm_available_numPages();//初始化时为120
...
void print_table_stats(const hash_table *table UNUSED) {
  ...
  printf("set_success:            %10zu | ", store->tstats.set_success);
...
void redistribute_last_short_group(hash_table *table, size_t *parts, size_t count) {
  ...
  while (k >= 0) {
    ...
    for (bucket_index = 0; bucket_index <= BUCKETS_PER_PARTITION; bucket_index++) {//要调整的group里面的block
      write_lock_bucket(&partition[bucket_index]);//此partition指block
