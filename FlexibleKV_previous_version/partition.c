...
uint64_t num_long_arcs_[2];//所有小group(默认)中的block数
uint64_t num_short_arc_groups_[2];//大group数(> S_ && < 2*S_)
uint64_t num_short_arcs_[2];//所有大group中的block数
uint64_t current_s_[2];//当前存在最小group_size
uint64_t S_;//group_size下限,上限为double S_
uint64_t S_minus_one_;
uint64_t S_log_;
uint64_t arc_groups_[2];//总group数
uint64_t num_buckets_;//Unused
uint64_t a_, b_;//Unused,Source code unused, too!

uint64_t lh_n, lh_n0, lh_l, lh_p, lh_x;////Unused,Source code use these for LinearHash()(not used here)
...
void NewBucket(Cbool version) {
  ...
  if (NumBuckets_v(version) < S_) {//起步用，只用8次
    // printf("S_:%zu\n",S_);
    num_long_arcs_[version]++;
    return;
  }
  // printf("\nBegin:long_arc:%zu,short_arc:%zu\n",num_long_arcs_[version],num_short_arcs_[version]);
  num_long_arcs_[version] -= current_s_[version];//long_arc减s_，小group数减1
  num_short_arc_groups_[version]++;//大group数加1
  num_short_arcs_[version] += current_s_[version] + 1;//short_arc加s_+1,block总数+1
  // printf("Step1:long_arc:%zu,short_arc:%zu,current_s_:%zu\n",num_long_arcs_[version],num_short_arcs_[version],current_s_[version]);
  ...
  if (num_long_arcs_[version] == 0) {
    ...
    current_s_[version]++;
    // printf("- - - - - - CIRCLE - - - - - -\n");
    // printf("Step2:long_arc:%zu,short_arc:%zu,current_s_:%zu\n",num_long_arcs_[version],num_short_arcs_[version],current_s_[version]);
...
void hash_init_partition(const Cbool version, uint64_t S, uint64_t num_buckets) {
  ...
  current_s_[version] = S_;
  arc_groups_[version] = 1;
  num_buckets_ = num_buckets;//Unused
  S_minus_one_ = S_ - 1;
  S_log_ = 0;
  while ((1UL << S_log_) < S_) {
    S_log_++;//log2(S_)
  ...
  lh_l = 0;
  lh_x = S_log_;//Unused
  while ((num_buckets >> lh_l) > 2 * S - 1) {//不发生split的group_size(上限)=15
    lh_l++;//得到log2(group数)
  }
  lh_n = num_buckets >> lh_l;//Unused,含义:得到小于group上限的block数，应为120>>3=15(120/8=15)
  lh_p = num_buckets - (lh_n << lh_l);//Unused,含义:余数120-8×15=0
  //上述操作得到的结果:计算出当前block总数共可分为多少group,每个group内又有多少blcok
  //当前条件下共分为8个group，其中每个group内有15个block
...
void get_last_short_group_parts(size_t *parts, size_t *count, Cbool version) {
  *count = 0;
  ...
  if (num_short_arc_groups_[version] == 0) {
    ...
    if (current_s_[version] == S_) actual_count = (S_ << 1) < (max_arc_num + 1) ? (S_ << 1) : max_arc_num + 1;
