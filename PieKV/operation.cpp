...
extern double global_timebase;
extern double global_timediff;
MemPool *kMemPool;
...
void Piekv::print_trigger() {
    ...
    while (is_running_) {
        ...
        res[0] = (double)(log_->log_segments_[0]->table_stats_->count
                                                +log_->log_segments_[1]->table_stats_->count
                                                +log_->log_segments_[2]->table_stats_->count
                                                +log_->log_segments_[3]->table_stats_->count
                                                - pre_count[0]) / timediff;
        res[1] = (double)(log_->log_segments_[0]->table_stats_->get_found
                                                +log_->log_segments_[1]->table_stats_->get_found
                                                +log_->log_segments_[2]->table_stats_->get_found
                                                +log_->log_segments_[3]->table_stats_->get_found
                                                - pre_count[1]) / timediff;
        res[2] = (double)(log_->log_segments_[0]->table_stats_->set_success
                                                +log_->log_segments_[1]->table_stats_->set_success
                                                +log_->log_segments_[2]->table_stats_->set_success
                                                +log_->log_segments_[3]->table_stats_->set_success
                                                - pre_count[2]) / timediff;
        if (res[0] > 1e-9)  printf("\n[P]count:              %7.3lf /s\n", res[0] );
        if (res[1] > 1e-9)  printf("\n[P]get found:          %7.3lf /s\n", res[1] );
        if (res[2] > 1e-9)  printf("\n[P]set success:        %7.3lf /s\n", res[2] );
        if((res[0] > 1e-9)||(res[1] > 1e-9)||(res[2] > 1e-9)) printf("\n----------------------------------\n"); 
